...
####################################
SZ <- 1.1
UP <- 3000 # upstream of TSS
DOWN <- 200 # downstream of Gene end
...
plot_points <- function(gene, data.gr, ensdb.genes){
    ...
    p <- data  %>%
      ...
      geom_point(size = POINT.SZ, alpha = POINT.ALPHA) +
      scale_y_continuous(breaks = c(0, 2.5, 5, 7.5, 10, 12.5), lim = c(0, 12.5)) +
