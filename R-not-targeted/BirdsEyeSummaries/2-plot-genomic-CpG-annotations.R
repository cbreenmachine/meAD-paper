...
out <- fdrtool(data$stat, statistic = "normal", plot = F)
...
annotate_regions <- function(input.gr, anno.list='hg38_basicgenes'){

  # hg38_basicgenes or hg38_cpg
  my.anno <- build_annotations(genome = 'hg38', annotations = anno.list)

  # Run the annotation function
  input.annotated <- annotatr::annotate_regions(input.gr, my.anno,
                                                  min = 2, quiet = T)

  # Count how many annotate to something else
  none.of.the.above <- subsetByOverlaps(input.gr,
                                        input.annotated,
                                        invert = T)
  ...
  anno.df <- data.frame(input.annotated) %>%
  ...
  anno.counts.df <- rbind(anno.df, other.df) %>%
    group_by(Annotation) %>%
    dplyr::summarize(value = round(100 * n() / length(input.gr), 1)) %>%
