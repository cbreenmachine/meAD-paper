...
make_panel <- function(gene.list, data.gr, dmrs.df, ensdb.genes, ensdb){
  ...
  p2 <- make_figure(gene.list[2], data.gr, dmrs.df, ensdb.genes, ensdb, xlab = F, ylab=T, legend = F)
  p3 <- make_figure(gene.list[3], data.gr, dmrs.df, ensdb.genes, ensdb, xlab = F, ylab=T, legend = F)
  p4 <- make_figure(gene.list[4], data.gr, dmrs.df, ensdb.genes, ensdb, xlab = T, ylab=T, legend = F)

  common.legend <- get_common_legend()

  panel <- cowplot::plot_grid(p1, p2, p3, p4, ncol = 1)
...
gene.lists <- list(
  "also_diff_exp" = c("UMAD1", "ABCA1", "MME", "MME"),
  "highest_effect" = c("SIRPB1", "GNGT1", "NIPA2", "TDG"),
