...
# DMRs are not identifying

ODIR <- "../../Figs/Fig2/"
SAMPLES.PATH <- "../../DataRaw/masterSamplesheet.csv"
PIS.PATH <- "../../DataRaw/2023-01-24-Summaries-v4/DMRegions-pis.bed"
...
data <- fread(PIS.PATH) %>%
  dplyr::filter(one.large.effect | persistent.effect) %>%
...
dmr.range <- unique(data$dmr.id)[1:10]
